...
(That sentence in the docs is the key; it reflects that sklearn historically expects numeric input and the user must encode categoricals before fitting.)
...
For a plain decision tree where you want native categorical splits, R (rpart) historically provided more direct support.
...
Practical takeaway: If you must use scikit-learn and want faithful categorical splitting, you either:
...
B — Use target / frequency / CatBoost-style encoders (if you must stay in scikit-learn):
...
If you must stay inside scikit-learn, categories can be encoded using OneHotEncoder or libraries like category_encoders (TargetEncoder, CatBoostEncoder, etc.). One-hot encoding avoids ordering, while target-based encoders reduce dimensionality. The pros are that these methods work everywhere without installing extra libraries. However, one-hot encoding can create many extra columns (for example, zipCode_50xxx), and decision trees often ignore most of them if the category effect is subtle. Your results confirmed this: many one-hot features had zero importance, so this approach is okay but not ideal for high-cardinality features.
...
Target encoding replaces each category with the mean of the target variable, like the average house price for each zip code. This creates only one new column per feature and captures strong signals. In your results, using target encoding made the zip code feature the most important, improved RMSE significantly, and produced more meaningful splits in the decision tree. This is the strongest and cleanest method if you are restricted to scikit-learn and can’t use CatBoost or LightGBM. In short: target encoding is simple, compact, and effective.
